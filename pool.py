...
global polygon, token, config, pool


configfile = "config.json"

polygon = Web3(Web3.HTTPProvider("https://rpc-mainnet.maticvigil.com/"))
...
def gasPrice():
    ...
    if (config["autoGasPrice"]):
        return int(min(requests.get("https://gasstation-mainnet.matic.network/").json()["fast"], config["maxGasPrice"])*config["gasMultiplier"])
    else:
...
def submitWork(nonce, result, miner, feeHolder):
    ...
    try:
        ...
        tx = pool.functions._mint(int(nonce), result, Web3.toChecksumAddress(miner), int(config["poolFee"]), feeHolder).buildTransaction({'nonce': polygon.eth.get_transaction_count(config["address"]),'chainId': 137, 'gasPrice': _gas, 'from':config["address"]})
